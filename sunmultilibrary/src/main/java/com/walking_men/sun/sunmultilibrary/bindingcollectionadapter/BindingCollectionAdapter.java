...
public interface BindingCollectionAdapter<T> {
     ...
     * update itself based on that timlineList's changes. <br/> Note that the adapter will keep a direct
     * reference to the given timlineList. Any changes to it <em>must</em> happen on the main thread.
