...
import com.walking_men.sun.sunmultilibrary.bindingcollectionadapter.factories.BindingAdapterViewFactory;
import com.walking_men.sun.sunmultilibrary.bindingcollectionadapter.factories.BindingViewPagerAdapterFactory;
...
public class BindingCollectionAdapters {
    ...
    // AdapterView
    @BindingAdapter({"itemView", "items", "headerItem"})
    public static <T> void setAdapter(ListView adapterView, ItemViewArg<T> arg, List<T> items, ItemView headItem) {
        setAdapter(adapterView, arg, items, BindingAdapterViewFactory.DEFAULT, null, headItem);
    }

    @SuppressWarnings("unchecked")
    @BindingAdapter({"itemView", "items", "adapter", "headerItem", "itemIds"})
    public static <T> void setAdapter(ListView adapterView,
                                      ItemViewArg<T> arg, List<T> items,
        ...
        if (null != items) {
            ...
            if (adapter == null) {
                adapter = factory.create(adapterView, arg);
                adapterView.setAdapter(adapter);
                Utils.addHeader(adapterView, headerItem);
            }
            adapter.setItems(items);
            adapter.setItemIds(itemIds);
        }
