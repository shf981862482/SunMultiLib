 ...
 */
package com.walking_men.sun.sunmultilibrary.http.upload.impl;

import android.os.Handler;
import android.os.Message;

import com.walking_men.sun.sunmultilibrary.http.upload.UIProgressListener;
import com.walking_men.sun.sunmultilibrary.http.upload.impl.handler.ProgressHandler;
import com.walking_men.sun.sunmultilibrary.http.upload.impl.model.ProgressModel;
 ...
 * Time: 22:34
 */
public abstract class PostProgressListener implements UIProgressListener {
    private boolean isFirst = false;

    //处理UI层的Handler子类
    private static class UIHandler extends ProgressHandler {

        public UIHandler(PostProgressListener uiProgressListener) {
            super(uiProgressListener);
        }

        @Override
        public void start(PostProgressListener uiProgressListener, long currentBytes, long contentLength, boolean done) {
        ...
        @Override
        public void progress(PostProgressListener uiProgressListener, long currentBytes, long contentLength, boolean done) {
            if (uiProgressListener!=null){
        ...
        @Override
        public void finish(PostProgressListener uiProgressListener, long currentBytes, long contentLength, boolean done) {
            if (uiProgressListener!=null){
